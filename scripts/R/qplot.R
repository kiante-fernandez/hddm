...
qplot <- function(data) {
  ...
  sim_rt <- data %>%
    ...
    summarise(
      rtQ1 = quantile(abs(rt), probs = seq(.1, .9, .2))[1],
      rtQ3 = quantile(abs(rt), probs = seq(.1, .9, .2))[2],
      rtQ5 = quantile(abs(rt), probs = seq(.1, .9, .2))[3],
      rtQ7 = quantile(abs(rt), probs = seq(.1, .9, .2))[4],
      rtQ9 = quantile(abs(rt), probs = seq(.1, .9, .2))[5]
    ) %>%
