...
C type (face/car), RT, correct vs error.
      ...
      character(8) aa
      d1="/u/russ/diff4/pb1-fast-dm/subj00"
      d4="/u/russ/diff4/pb1-fast-dm/subj0"
C     d2=".pb1.p0_N0200M0600.fast-dm.csv"
      d2=".pb1.p4_N0200M0600.fast-dm.csv"
      d1=adjustl(d1)
      d4=adjustl(d4)
      d2=adjustl(d2)
      d1=trim(d1)
...
C     WRITE(4,47)(X(I),I=1,NV)
   47 FORMAT(f7.4,9F8.4)
    ...
    3 if(rt(i).gt.pxa)pxa=rt(i)
      ...
      if(.9*x(4).gt.z.or..9*x(4).gt.a-z)x(4)=min(1.8*z,1.8*(a-z))
      sg=x(4)
      xmlh=0.
!$omp parallel
