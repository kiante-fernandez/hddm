...
c      include '/opt/intel/Compiler/11.1/064/mkl/include/mkl_vsl.fi'
      ...
      double precision rt(8000),yi(8000),cond(8000),gu(80000),y(39)
      integer mch(8000),mcond(8000),con(8000)
      character(95) d1,d2,ff,d3,d4,ff3,ff4,dup,ddo
...
C      nn=400
      ...
      ict=1
c NS is number of subjects     
      ns=1
      do 7 k=1,ns
      write(d3,"(i2)")k
      d3=adjustl(d3)
      ff3=trim(dup)//trim(d1)//trim(d3)//trim(d2)
      if(k.gt.9)ff3=trim(dup)//trim(d4)//trim(d3)//trim(d2)
      ff4=trim(ddo)//trim(d1)//trim(d3)//trim(d2)
      if(k.gt.9)ff4=trim(ddo)//trim(d4)//trim(d3)//trim(d2)
      open(3,file=trim(ff3))
      open(4,file=trim(ff4))
C     do 1 i=1,n
C     read(1,*)mcond(i),mch(i),rt(i)
C     print*,ich,rr,aa
C     rt(i)=1000.*rt(i)
C     mch(i)=ich
C     if(aa.eq."high")mcond(i)=1
C     if(aa.eq."low")mcond(i)=2
C     if(aa.eq."vlow")mcond(i)=3
C     if(aa.eq."nonword")mcond(i)=4
C     write(*,"(f8.3,2i5,f8.3,i5)")yi(i),mcond(i),con(i),rt(i),mch(i)
C   1 continue
C     OPEN(14,STATUS='SCRATCH')
C     OPEN(15,STATUS='SCRATCH')
C     OPEN(16,STATUS='SCRATCH')
...
C files
C     write(14,*)n,(mcond(i),i=1,n)
C     write(*,*)n,(mcond(i),i=1,n)
C     write(15,*)n,(rt(i),i=1,n)
C     write(*,*)n,(rt(i),i=1,n)
C     write(16,*)n,(mch(i),i=1,n)
C     write(*,*)n,(mch(i),i=1,n)
C     rewind(14)
C     rewind(15)
C     rewind(16)
      x(1)=.11
      ...
      x(6)= .10
      x(7)= .0001
C a ter (eta) sz z st po then set of slope + intercepts.
      x(8)= .3
      x(9)= .20
      x(10)= .1
      x(11)=-.1
...
C     WRITE(4,47)(X(I),I=1,NV)
   47 FORMAT(f7.4,9F8.4)
    7 CONTINUE
      ...
      double precision yi(8000),xml(8000)
      integer mch(8000),mcond(8000),con(8000),rt(8000)
      nn=8000
      n=nn/4
      kk=0
      do 2 j=1,4
      read(3,*)n1
      read(3,*)(rt(i+kk),i=1,n1-1)
C     print*,rt(1+kk),rt(n1-1+kk),n1-1
      read(4,*)n2
      read(4,*)(rt(i+kk),i=n1,n1+n2-2)
C     print*,j,kk,n1,n2
      do 4 jk=kk+1,kk+n1-1
      mcond(jk)=j
      mch(jk)=1
    4 continue
      do 5 jk=kk+n1,kk+n1+n2-2
      mcond(jk)=j
      mch(jk)=2
    5 continue
      kk=kk+n1+n2-2
    2 continue
      rewind(3)   
      rewind(4)
C     write(*,"(4i5)")(i,mcond(i),mch(i),rt(i),i=1990,2010)
C     write(*,"(4i5)")(i,mcond(i),mch(i),rt(i),i=3990,4010)
C     write(*,"(4i5)")(i,mcond(i),mch(i),rt(i),i=5990,6010)
C     write(*,"(4i5)")(i,mcond(i),mch(i),rt(i),i=7990,8000)
C     write(88,"(4i5)")(i,mcond(i),mch(i),rt(i),i=1,8000)
C     read(14,*)n,(mcond(i),i=1,nn)
C     read(15,*)n,(rt(i),i=1,nn)
C     write(*,*)n,(rt(i),i=1,nn)
C     read(16,*)n,(mch(i),i=1,nn)
C     rewind(14)
C     rewind(15)
C     rewind(16)
      pxa=0.
      do 3 i=1,nn
    3 if(rt(i)*1..gt.pxa)pxa=rt(i)*1.
      pxa=pxa/1000.
      s=.1
      if(x(7).lt.0.0D0)x(7)=0.0D0
      x(7)=0.0001D0
      pz=x(7)
      m=5
      mm=1
...
C     if(x(7+jj).lt.-0.7)x(7+jj)=-0.7
      ...
      rr=1.*rt(j)/1000.
C     write(*,"(4i5,8f7.3)")j,jj,mcond(j),mch(j),v,a,zz,rr,pxa,terr
...
C     z=z+sg*gw
      a=a+sg*gw
      z=a/2.
C modify z to be a proportion of a,  THis for future z ne a/2
C     z=a*zz
